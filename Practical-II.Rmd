...
Where both $\epsilon$ and $\gamma$ are constant probabilities.
...
### Simple metapopulation model (MacKenzie like) - with time-dependent colonization rate

How could write a model with time dependent colonization rate ? Could that be used for projections ?
...
c_boot = list( inv.logit(c_related_param[[1]] %*% t(envt_matrix) + gamma_c[1]),
               inv.logit(c_related_param[[2]] %*% t(envt_matrix) + gamma_c[2]) )

e_boot = list( inv.logit(e_related_param[[1]] %*% t(envt_matrix) + gamma_e[1]),
               inv.logit(e_related_param[[2]] %*% t(envt_matrix) + gamma_e[2]) )
