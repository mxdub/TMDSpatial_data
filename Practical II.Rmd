...
plot_envt_effect(data_envt$env1, c_boot, e_boot, species)
...
sim = simulate_MC(patches = n_patch,     # Number of patch
                  ...
                  env1Scale = 100,       # Low spatial autocorrelation for envt (not important)
                  env_optima = envt_opt, # Environmental effects are Gaussian around optimum
                  env_niche_breadth = 0.1, # Niche breadth (~ sd of the Gaussian)
                  int_mat = matrix( c(0.05,0.07,
...
for(s in 1:n_species){
  for(r in 1:n_rep){
    ...
    for(i in 2:(serie_length+time_out_fit))
      # Simulate patch dynamic (including uncertainty from the posterior distributions)
      z[s,,i] = sapply(z[s,,i-1]*(1-inv.logit((e_related_param[[s]][sample(1:samples_length, 1),] %*% t(envt_matrix))[1,]))+
                    (1-z[s,,i-1])*inv.logit((c_related_param[[s]][sample(1:samples_length, 1),] %*% t(envt_matrix))[1,]),
                    FUN = function(x) rbinom(1,1,x) )
    rep[s,r,] = apply(z[s,,], 2, mean)
  }
}
...
for(s in 1:n_species){
  for(r in 1:n_rep){
    ...
    for(i in 2:(serie_length+time_out_fit))
      # Simulate patch dynamic (including uncertainty from the posterior distributions)
      z[s,,i] = sapply(z[s,,i-1]*(1-inv.logit((e_related_param[[s]][sample(1:samples_length, 1),] %*% t(envt_matrix))[1,]))+
                    (1-z[s,,i-1])*(1-exp(-(exp(c_related_param[[s]][sample(1:samples_length, 1),] %*% t(envt_matrix))[1,]*mean(z[s,,i-1]))) ),
                    FUN = function(x) rbinom(1,1,x) )
    rep[s,r,] = apply(z[s,,], 2, mean)
  }
}
